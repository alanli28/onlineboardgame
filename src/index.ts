...
function useAction(state: GameState, player: Player, actionId: ActionId, cardIds: string[]): void {
	...
	if (state.activePlayer !== player) throw new Error(`It is ${playerName(state.activePlayer)}'s turn.`);
	...
	if (actionId === "secret") {
		...
		addLog(state, `${playerName(player)} used Secret.`);
		finishAction(state, player);
		return;
	}
	if (actionId === "tradeOff") {
		state.players[player].tradeOff = [...cardIds];
		addLog(state, `${playerName(player)} used Trade-off.`);
		finishAction(state, player);
		return;
	}
	if (actionId === "gift") {
...
function chooseGift(state: GameState, player: Player, cardId: string): void {
	...
	addLog(state, `${playerName(player)} chose 1 Gift card.`);
...
function chooseCompetition(state: GameState, player: Player, setIndex: number): void {
	...
	addLog(state, `${playerName(player)} chose a Competition set.`);
	finishOffer(state, offer.from);
}
...
function scoreRound(state: GameState): void {
	...
	if (p1Goal || p2Goal) {
		...
		addLog(state, `${playerName(winner)} wins with ${state.winReason}.`);
	} else {
...
function dealNextRound(state: GameState, startingPlayer: Player, resetMarkers: boolean): GameState {
	...
	state.deck = deck;
	addLog(state, `Round ${state.round} started. ${playerName(startingPlayer)} starts.`);
	drawForActivePlayer(state);
	touch(state);
	return state;
}
...
function drawForActivePlayer(state: GameState): void {
	...
	addLog(state, `${playerName(state.activePlayer)} drew a card.`);
}
...
function assertPendingChoice(state: GameState, player: Player, type: Offer["type"]): void {
	...
	if (state.offer.to !== player) throw new Error(`${playerName(state.offer.to)} must make this choice.`);
}
...
function playerName(player: Player | null): string {
	return player === "p1" ? "Player 1" : player === "p2" ? "Player 2" : "No player";
}
...
function body() {
	...
	return \`<section class="status-strip"><div><span class="label">Room</span><strong>\${esc(room)}</strong></div><div><span class="label">You</span><strong>\${name(player)}</strong></div><div><span class="label">Turn</span><strong>\${state.activePlayer ? name(state.activePlayer) : state.status}</strong></div><div><span class="label">Score</span><strong>P1 \${scores.p1.geishas}/\${scores.p1.points} - P2 \${scores.p2.geishas}/\${scores.p2.points}</strong></div></section><div class="game-layout"><section class="board-panel"><div class="section-title"><h2>Board</h2><span>\${state.deckCount} card(s) left in deck</span></div><div class="geisha-grid">\${state.geishas.map(renderGeisha).join("")}</div></section><aside class="side-panel">\${playerPanel()}\${logPanel()}</aside></div>\`;
}
function renderGeisha(geisha) { const owner = state.markers[geisha.id]; return \`<article class="geisha-card" style="--accent:\${geisha.color}"><div class="played-zone top">\${smallCards(cardsFor("p2", geisha.id))}</div><div class="geisha-main"><span class="card-corner top-left">\${geisha.value}</span><span class="card-corner bottom-right">\${geisha.value}</span><span class="card-pattern"></span><span class="geisha-crest">\${geisha.monogram}</span><span class="name">\${geisha.name}</span><span class="item-name">\${geisha.item}</span><span class="marker \${owner || "center"}">\${owner ? name(owner) : "Center"}</span></div><div class="played-zone">\${smallCards(cardsFor("p1", geisha.id))}</div></article>\`; }
function playerPanel() { const me = state.players[player]; const other = state.players[player === "p1" ? "p2" : "p1"]; return \`<section class="panel"><div class="section-title"><h2>\${name(player)}</h2><span>\${other.handCount} card(s) in opponent hand</span></div>\${instructions()}\${actions(me)}\${hand(me)}<div class="private-grid"><div><h3>Secret</h3><p>\${me.secret.length ? me.secret.map(label).join(", ") : "Not used"}</p></div><div><h3>Trade-off</h3><p>\${me.tradeOff.length ? me.tradeOff.map(label).join(", ") : "Not used"}</p></div></div></section>\`; }
...
function cardFace(id, showValue) { const geisha = geishaFor(id); const corner = showValue ? geisha.value : geisha.monogram; return \`<span class="card-corner top-left">\${corner}</span><span class="card-corner bottom-right">\${corner}</span><span class="card-pattern"></span><span class="card-center"><b>\${geisha.item}</b><em>\${geisha.name}</em></span>\`; }
...
function handLabel(id) { return geishaFor(id).item; }
function label(id) { const geisha = geishaFor(id); return \`\${geisha.item} \${geisha.value}\`; }
...
.item-name{position:relative;z-index:1;color:#6a5d50;font-size:.74rem;font-weight:800;letter-spacing:.06em;text-transform:uppercase}
...
.action-card[data-disabled="true"]{cursor:not-allowed;opacity:.62}
.action-card.used{opacity:.5}
...
.card-center em{color:#6a5d50;font-size:.7rem;font-style:normal;font-weight:800;letter-spacing:.04em;text-transform:uppercase}
