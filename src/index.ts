...
type Geisha = { id: string; name: string; value: number; color: string };
type PlayerState = {
	hand: string[];
	tableau: string[];
	secret: string[];
...
const GEISHAS: Geisha[] = [
	{ id: "violet", name: "Violet", value: 2, color: "#8b5cf6" },
	{ id: "red", name: "Red", value: 2, color: "#ef4444" },
	{ id: "lime", name: "Lime", value: 2, color: "#84cc16" },
	{ id: "blue", name: "Blue", value: 3, color: "#3b82f6" },
	{ id: "amber", name: "Amber", value: 3, color: "#f59e0b" },
	{ id: "green", name: "Green", value: 4, color: "#10b981" },
...
document.addEventListener("click", async (event) => {
	...
	if (action === "use-action") return move({ type: "useAction", action: el.dataset.gameAction, cardIds: selected });
...
function renderGeisha(geisha) { const owner = state.markers[geisha.id]; return \`<article class="geisha-card" style="--accent:\${geisha.color}"><div class="played-zone top">\${smallCards(cardsFor("p2", geisha.id))}</div><div class="geisha-main"><span class="value">\${geisha.value}</span><span class="name">\${geisha.name}</span><span class="marker \${owner || "center"}">\${owner ? name(owner) : "Center"}</span></div><div class="played-zone">\${smallCards(cardsFor("p1", geisha.id))}</div></article>\`; }
